...
import { GenreType, AudienceType } from '../../types';
...
export const createCharacterTemplate: PromptTemplate<CreateCharacterParams> = ({
  genre,
  audience,
  role = 'character',
  name,
  ...
  setting_description,
  related_characters = [],
  narrative_importance = 'supporting',
}) => `
...
// Template for expanding an existing character
export interface ExpandCharacterParams extends CreateCharacterParams {
  existing_character: {
    name: string;
    description: string;
    traits?: string[];
  ...
  expansion_focus: 'background' | 'relationships' | 'development' | 'details';
}

export const expandCharacterTemplate: PromptTemplate<ExpandCharacterParams> = ({
  genre,
  audience,
  existing_character,
  expansion_focus,
